...
tape("stats-nist", function(t) {

  // From NIST Statistical Reference Datasets
  ...
  var pidigits = [ 3, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5, 8,
  ...
  var numacc1 = [10000001, 10000003, 10000002];
  var numacc2 = new Array(1001);
  var numacc3 = new Array(1001);
  var numacc4 = new Array(1001);
  ...
  t.nearequal(st.mean(lew), -177.435000000000, 1e-15, "lew-mean");
  t.nearequal(st.std(lew), 277.332168044316, 1e-15, "lew-std");

  t.nearequal(st.mean(lottery), 518.958715596330, 1e-15, "lottery-mean");
  t.nearequal(st.std(lottery), 291.699727470969, 1e015, "lottery-std");

  t.nearequal(st.mean(mavro), 2.00185600000000, 1e-15, "mavro-mean");
  t.nearequal(st.std(mavro), 0.000429123454003053, 1e-13, "mavro-std");

  t.nearequal(st.mean(michelson), 299.852400000000, 1e-15, "michelson-mean");
  t.nearequal(st.std(michelson), 0.0790105478190518, 1e-13, "michelson-std");

  t.nearequal(st.mean(pidigits), 4.53480000000000, 1e-14, "pidigits-mean");
  t.nearequal(st.std(pidigits), 2.86733906028871, 1e-15, "pidigits-std");

  t.nearequal(st.mean(numacc1), 10000002, 1e-15, "numacc1-mean");
  t.nearequal(st.std(numacc1), 1, 1e-15, "numacc1-std");

  t.nearequal(st.mean(numacc2), 1.2, 1e-15, "numacc2-mean");
  t.nearequal(st.std(numacc2), 0.1, 1e-15, "numacc2-std");

  t.nearequal(st.mean(numacc3), 1000000.2, 1e-15, "numacc3-mean");
  t.nearequal(st.std(numacc3), 0.1, 1e-9, "numacc3-std");

  t.nearequal(st.mean(numacc4), 10000000.2, 1e-15, "numacc4-mean");
  t.nearequal(st.std(numacc4), 0.1, 1e-7, "numacc4-std");
